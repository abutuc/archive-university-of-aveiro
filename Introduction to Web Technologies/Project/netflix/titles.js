...
var vm = function () {
    ...
    self.displayName = ko.observable('Titles');
    ...
    self.titles = [
        {name: "All Titles", value:"AllTitles"},
        {name: "Last Titles", value:"LastTitles"},
        {name: "2020", value:"2020"},
        {name: "2019", value:"2019"},
        {name: "2018", value:"2018"}
    ];

    self.chosenTitle = ko.observable();

    self.TitleSelected = function(){
        switch (self.chosenTitle()['value']){
            case "AllTitles":
                self.baseUri('http://192.168.160.58/netflix/api/titles?')
                self.currentPage = ko.observable(1);
                self.hasPrevious = ko.observable(false);
                self.hasNext = ko.observable(false);
            ...
            case "LastTitles":
                console.log("last titles!!");
                self.baseUri('http://192.168.160.58/netflix/api/LastTitles?')
                self.currentPage = ko.observable(1);
                self.hasPrevious = ko.observable(false);
                self.hasNext = ko.observable(false);
                self.displayName("Last Titles");
                self.activate(1);
                break;

            case "2020":
                self.baseUri('http://192.168.160.58/netflix/api/TitlesByYear?year=2020&')
                self.currentPage = ko.observable(1);
                self.hasPrevious = ko.observable(false);
                self.hasNext = ko.observable(false);
                self.displayName("2020");
                self.activate(1);
                break;
            
            case "2019":
                self.baseUri('http://192.168.160.58/netflix/api/TitlesByYear?year=2019&')
                self.currentPage = ko.observable(1);
                self.hasPrevious = ko.observable(false);
                self.hasNext = ko.observable(false);
                self.displayName("2019");
                self.activate(1);
                break;
            
            case "2018":
                self.baseUri('http://192.168.160.58/netflix/api/TitlesByYear?year=2018&')
                self.currentPage = ko.observable(1);
                self.hasPrevious = ko.observable(false);
                self.hasNext = ko.observable(false);
                self.displayName("2018");
                self.activate(1);
                break;
        }
    }
