...
$(document).ready(function () {
    ...
    function doneTyping () {
        ...
        console.log(input);
        $.ajax({
            url: 'http://192.168.160.58/netflix/api/Search/Countries?name=' + input.toString(),
            type: 'GET',
            dataType: 'json',
            ...
            success: function(data){
                ...
                if (result.length != 0){
                    for (var i = 0; i < result.length; i++){
                        string += "<a href='./countryDetails.html?id="+ ids[i] + "&page=1&pagesize=20&titles=" + titles[i]  + "'>" + result[i] + '</a>' + '<br>';
                    }
                }
                    else {
                        string = "<h5>Sorry...we don't have the Country you searched for <i style='padding-left:2%' class='fa fa-frown-o fa-2x' aria-hidden='true'></i></h5>";
                    }
                $('#ModalLabel').text(result.length +"  Countries Searched");
                $('#CountriesSearched').html(string);
