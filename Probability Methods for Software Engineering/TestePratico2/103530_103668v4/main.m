...
while (op>1 || op<5) 
    ...
    elseif op==2
        ...
        similar(user_id,user_data,friends_min_hash);
    elseif op==3
        k=150;%number of hash functions
        tam_shingle=3; %shingle
        limiar=0.8; %threshold=0.8 dado do enunciado
        find_name(user_data,names_min_hash,tam_shingle,limiar);
    elseif op==4
        k=150;
        tam_shingle=3;
        similar_interesses(user_id,user_data,friends,interesses_min_hash);
    elseif op==5
