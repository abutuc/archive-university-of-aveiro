...
public class Dip {
    ...
    public static Dip generateRandomDip()  {
        ...
        int numberCandidatesAdded = 0;
        while (numberCandidatesAdded < NUMBERS_REQUIRED){
            int candidate = generator.nextInt(NUMBERS_RANGE_MAX) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                numberCandidatesAdded++;
            }
        }

        int starsCandidatesAdded = 0;
        while (starsCandidatesAdded < STARS_REQUIRED){
