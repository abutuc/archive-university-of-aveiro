# A set of functions to deal with bags of money.
#
# JMR 2017

# Face values of coins (in cents):
COINS = [200, 100, 50, 20, 10, 5, 2, 1]

def value(bag):
    """Return total amount in a bag."""
    amount = 0
    for coin in bag:
        amount += coin * bag[coin]
    return amount

def transfer1coin(bag1, c, bag2):
    """Try to transfer one coin of value c from bag1 to bag2.
    If possible, transfer coin and return True, otherwise return False."""
    
    if bag1[c] != 0:
        bag1[c] -= 1
        if c not in bag2:
            bag2[c] = 1
        else:
            bag2[c] += 1
        return True
    else:
        return False


def transfer(bag1, amount, bag2):
    """Try to transfer an amount from bag1 to bag2.
    If possible, transfer coins and return True,
    otherwise, return False and leave bags with same values."""
    if amount == 0:
        return True
    if value(bag1) < amount:
        return False
    else:
        if 200 in bag1:
            coins_200 = amount // 200
            after_200 = amount % 200
        else:
            coints_200 = 0
            after_200 = amount
        
        if 100 in bag1:
            coins_100 = after_200 // 100
            after_100 = after_200 % 100
        else:
            coins_100 = 0
            after_100 = after_200
        
        if 50 in bag1:
            coins_50 = after_100 // 50
            after_50 = after_100 % 50
        else:
            coins_50 = 0
            after_50 = after_100
        
        if 20 in bag1:
            coins_20 = after_50 // 20
            after_20= after_50 % 20
        else:
            coins_20 = 0
            after_20 = after_50
        
        if 10 in bag1:
            coins_10 = after_20 // 10
            after_10 = after_20 % 10
        else:
            coins_10 = 0
            after_10 = after_20
        
        if 5 in bag1:
            coins_5 = after_10 // 5
            after_5 = after_10 % 5
        
        if 2 in bag1:
            coins_2 = after_5 // 2
            after_2 = after_5 % 2
        else:
            coins_2 = 0
            after_2 =after_5

        coins_1 = after_2

        if 200 in bag1:
            bag1[200] -= coins_200
        else:
            bag1[200] = coins_200
        
        if 200 in bag2:
            bag2[200] += coins_200
        else:
            bag2[200] = coins_200
        
        if 100 in bag1:
            bag1[100] -= coins_100
        else:
            bag1[100] = coins_100
        
        if 100 in bag2:
            bag2[100] += coins_100
        else:
            bag2[100] = coins_100
        
        if 50 in bag1:
            bag1[50] -= coins_50
        else:
            bag1[50] = coins_50
        
        if 50 in bag2:
            bag2[50] += coins_50
        else:
            bag2[50] = coins_50

        if 20 in bag1:
            bag1[20] -= coins_20
        else:
            bag1[20] = coins_20
        
        if 20 in bag2:
            bag2[20] += coins_20
        else:
            bag2[20] = coins_20
        
        if 10 in bag1:
            bag1[10] -= coins_10
        else:
            bag1[10] = coins_10
        
        if 10 in bag2:
            bag2[10] += coins_10
        else:
            bag2[10] = coins_10

        if 5 in bag1:
            bag1[5] -= coins_5
        else:
            bag1[5] = coins_5
        
        if 5 in bag2:
            bag2[5] += coins_5
        else:
            bag2[5] = coins_5
        
        if 2 in bag1:
            bag1[2] -= coins_2
        else:
            bag1[2] = coins_2
        
        if 2 in bag2:
            bag2[2] += coins_2
        else:
            bag2[2] = coins_2
        
        if 1 in bag1:
            bag1[1] -= coins_1
        else:
            bag1[1] = coins_1
        
        if 1 in bag2:
            bag2[1] += coins_1
        else:
            bag2[1] = coins_1

        return True
    
            

def strbag(bag):
    """Return a string representing the contents of a bag.""" 
    # You may want to change this to produce a more user-friendly
    # representation such as "4x200+3x50+1x5+3x1=958".
    table = dict(sorted(bag.items(), key= lambda item: item , reverse=True))
    string = ''
    for key in table:
        if table[key] != 0:
            string += "{}x{}+".format(table[key], key)
    string = string[:len(string)-1]
    string += "={}".format(value(bag))
    return string
    


def main():
    # A bag of coins is represented by a dict of {coin: number} items
    bag1 = {1: 4, 2: 0, 5:1, 10: 0, 20: 5, 50: 4, 100: 2, 200: 1}
    bag2 = {}

    # Test the value function.
    assert value({}) == 0
    assert value({1:7, 5:2, 20:4, 100:1}) == 197

    # Test the strbag function.
    print( strbag({1:7, 5:2, 20:4, 100:1}) )        # 1x100+4x20+2x5+7x1=197
    print( strbag({1:7, 5:2, 10:0, 20:4, 100:1}) )  # 1x100+4x20+2x5+7x1=197

    print("bag1:", strbag(bag1))    # bag1: 1x200+2x100+4x50+5x20+1x5+4x1=709
    print("bag2:", strbag(bag2))    # bag2: =0
    
    print(transfer1coin(bag1, 10, bag2))    # False!
    print("bag1:", strbag(bag1))    # bag1: 1x200+2x100+4x50+5x20+1x5+4x1=709
    print("bag2:", strbag(bag2))    # bag2: =0

    print(transfer1coin(bag1, 20, bag2))    # True
    print("bag1:", strbag(bag1))    # bag1: 1x200+2x100+4x50+4x20+1x5+4x1=689
    print("bag2:", strbag(bag2))    # bag2: 1x20=20

    print(transfer1coin(bag1, 20, bag2))    # True
    print("bag1:", strbag(bag1))    # bag1: 1x200+2x100+4x50+3x20+1x5+4x1=669
    print("bag2:", strbag(bag2))    # bag2: 2x20=40

    print(transfer(bag1, 157, bag2))        # True (should be easy)
    print("bag1:", strbag(bag1))    # bag1: 1x200+1x100+3x50+3x20+2x1=512
    print("bag2:", strbag(bag2))    # bag2: 1x100+1x50+2x20+1x5+2x1=197

    print(transfer(bag1, 60, bag2)) # not easy, but possible...
    print("bag1:", strbag(bag1))
    print("bag2:", strbag(bag2))

    return

if __name__ == "__main__":
    main()

