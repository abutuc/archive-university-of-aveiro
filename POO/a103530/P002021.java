...
public class P002021 {
	...
	private static void alinea2(PrintStream out) {
		...
		try{
			Scanner sc = new Scanner(new File("a103530/uafest.txt"));
			int pointer = 0;
			while(sc.hasNextLine()){
				String[] l_content = sc.nextLine().split(";");
				if(pointer == 0){
					fest = new AgenciaTuristica(l_content[0], l_content[1]);
					pointer++;
				}
				else{
					if (l_content[0].equals("1")){
						fest.add(new Gastronomia(Integer.parseInt(l_content[1]), l_content[2]));
					}
					else if (l_content[0].equals("2")){
						String[] locais = new String[l_content.length - 3];
						for (int i = 0; i<locais.length; i++){
							locais[i] = l_content[3+i];
						}
						fest.add(new PasseioBicicleta(Integer.parseInt(l_content[1]), l_content[2], locais));
					}
				}
			}
			sc.close();
			...
			System.out.println("O ficheiro não foi encontrado com sucesso.");
		}
