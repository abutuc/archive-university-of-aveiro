...
class BayesNet:
    ...
    def _list_conjunctions(self,variables):
        if len(variables) == 1:
            return [ [(variables[0],True)], [(variables[0],False)]]
        
        temp = []
        for s in self._list_conjunctions(variables[1:]):
            temp.append([(variables[0],True)]+s)
            temp.append([(variables[0],False)]+s)
        return temp
    
    def individualProb(self,var,val):
        all_conj = self._list_conjunctions(list(self.dependencies.keys()))
        return sum([self.jointProb(c) for c in all_conj if (var,val) in c])
