    ...
    def search(self, limit=None):
        while self.open_nodes != []:
            ...
            if self.problem.goal_test(node.state):
                ...
                self.average_depth = self.depth_total/(self.non_terminals+self.terminals)
            ...
            for a in self.problem.domain.actions(node.state):
                ...
                if not node.in_parent(newstate) and (limit is None or node.depth < limit):
                    ...
                    if len(self.highest_cost_nodes) == 0:
                        self.highest_cost_nodes.append(newnode)
                    else:
                        if newnode.cost > max([n.cost for n in self.highest_cost_nodes]):
                            self.highest_cost_nodes.clear()
                            self.highest_cost_nodes.append(newnode)
                        elif newnode.cost == max([n.cost for n in self.highest_cost_nodes]):
                            self.highest_cost_nodes.append(newnode)
                    lnewnodes.append(newnode)
                    self.depth_total += newnode.depth
