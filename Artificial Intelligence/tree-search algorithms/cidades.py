...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        for conn in self.connections:
            if (C1 == conn[0] and C2 == conn[1]) or (C1==conn[1] and C2==conn[0]):
                return conn[2]
        return None
    
    def heuristic(self, city, goal_city):
        (x_coordinates_city, y_coordinates_city) = self.coordinates[city]
        (x_coordinates_goal_city, y_coordinates_goal_city) = self.coordinates[goal_city]
        return math.sqrt(math.pow((y_coordinates_goal_city - y_coordinates_city), 2) + math.pow((x_coordinates_goal_city-x_coordinates_city), 2))
