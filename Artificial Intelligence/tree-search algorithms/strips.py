...
class STRIPS(SearchDomain):
    ...
    # se para todas as precondições elas estão dentro do estado
    def result(self, state, action):
        new_state = [c for c in state if not c in action.neg]
        return new_state + action.pos
        if not all(pred in state for pred in action.pc):
            return None
        state = [st for st in state if st not in action.neg]
