...
class FingerTable:
    ...
    def __init__(self, node_id, node_addr, m_bits=10):
        ...
        self._size = m_bits
        # Table entries: [node_id, node_addr]
        self._table = [[(self._node_id + 2**i) % (2**self._size), ()] for i in range(self._size)]

    def fill(self, node_id, node_addr):
        """Fill all entries of finger_table with node_id, node_addr."""
        for i in range(self._size):
            self._table[i][0] = node_id
            self._table[i][1] = node_addr

    def update(self, index, node_id, node_addr):
        """Update index of table with node_id and node_addr."""
        self._table[index - 1] = [node_id, node_addr]

    def find(self, identification):
        """Get node address of closest preceding node (in finger table) of identification."""
        for i in range(self._size - 1, -1, -1):
            if contains(self._node_id, identification, self._table[i][0]) and identification != self._table[i][0]:
                return self._table[i][1]
        return self._table[0][1] # This node's successor

    def refresh(self):
        """Retrieve finger table entries."""
        return [(i + 1, (self._node_id + 2**i) % (2**self._size), self._table[i][1]) for i in range(self._size)]

    def getIdxFromId(self, id):
        index = -1  # Not found
        for i in range(self._size):
            if self._table[i][0] == id:
                index = i + 1
                break
        return index
    ...
    def as_list(self):
        ...
        """
        return [tuple(row) for row in self._table]
...
class DHTNode(threading.Thread):
    ...
    def node_join(self, args):
        ...
        if self.identification == self.successor_id:  # I'm the only node in the DHT
            ...
            self.successor_addr = addr
            # TODO update finger table - Done
            for row in self.finger_table.refresh():
                if contains(self.identification, self.successor_id, row[1]):
                    self.finger_table.update(row[0], self.successor_id, self.successor_addr)
            #
            self.send(addr, {"method": "JOIN_REP", "args": args})
        else:
    ...
    def get_successor(self, args):
        ...
        if contains(self.identification, self.successor_id, identification):
            # Requested successor is between this node (exclusive) and this node's successor (inclusive)
            args = {"req_id": identification, "successor_id": self.successor_id, "successor_addr": self.successor_addr}
            self.send(addr, {"method": "SUCCESSOR_REP", "args": args})
        else:
            # Otherwise, propagate SUCCESSOR message to the nearest predecessor of id in the local finger_table
            self.send(self.finger_table.find(identification), {"method": "SUCCESSOR", "args": args})
        #
    ...
    def stabilize(self, from_id, addr):
        ...
        for row in self.finger_table.refresh():
            args = {"id": row[1], "from": self.addr}
            self.send(row[2], {"method": "SUCCESSOR", "args": args})
        #
        ...
        while not self.done:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REQ":
                    ...
                    for row in self.finger_table.refresh():
                        if row[1] == output["args"]["req_id"]:
                            self.finger_table.update(row[0], output["args"]["successor_id"], output["args"]["successor_addr"])
                            break
                    #
